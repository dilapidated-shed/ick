...
```c
E3M2 e3m2_from_code(uint8_t);
uint8_t e3m2_code(E3M2);
...
```c
E5M3 e5m3_from_code(uint8_t);
uint8_t e5m3_code(E5M3);
...
The header requires an eight-bit byte and IEEE-like binary32 `float`.  The
...
single-member structures exactly like `uint8_t`.  External ABI boundaries
should pass the payload code as `uint8_t` unless both sides deliberately
